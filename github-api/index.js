var $2x0gv$axios = require("axios");

function $parcel$interopDefault(a) {
  return a && a.__esModule ? a.default : a;
}

const $4adef408a494122b$var$createCard = (data) => `
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    let response = "";
    try {
      response = await $parcel$interopDefault($2x0gv$axios).get(
        `https://api.github.com/users/${username}`
      );
    } catch (error) {
      if (404 === error.response.status) alert("Username not found");
      else {
        alert("Error");
    ...
    if (response) {
      const card = $4adef408a494122b$var$createCard(response.data);
      document
